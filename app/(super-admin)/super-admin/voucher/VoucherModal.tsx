    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-[#0d0d0d] border border-white/10 p-8 rounded-[2.5rem] max-w-xl w-full shadow-2xl max-h-[90vh] overflow-y-auto no-scrollbar">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <button onClick={onClose} className="p-2 bg-white/5 rounded-full hover:bg-red-600 transition-all text-white"><X size={18} /></button>
        </div>
        ...
        <form onSubmit={(e) => { e.preventDefault(); if(canSubmit) onSubmit(formData); }} className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-black uppercase text-zinc-500 ml-1 flex items-center gap-2"><Activity size={12} /> Giới hạn dùng</label>
              <input 
          ...
          <div className="space-y-1.5">
            <label className="text-[10px] font-black uppercase text-zinc-500 ml-1 flex items-center gap-2">
              <LayoutGrid size={12} /> Thuộc sự kiện
            </label>
            <select
              name="promotionId"
              value={formData.promotionId}
              onChange={handleChange}
              className="w-full bg-zinc-900 border border-white/5 p-3.5 rounded-xl text-white text-sm font-bold outline-none focus:border-red-600 appearance-none"
            >
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-black uppercase text-zinc-500 ml-1">Đơn tối thiểu</label>
              <input 
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <input 
                ...
                className="w-full bg-zinc-900 border border-white/5 p-3 rounded-xl text-[11px] text-white font-bold outline-none focus:border-red-600 [color-scheme:dark]" 
              />
            </div>
            <div className="space-y-1.5">
              ...
              <input 
                ...
                className={`w-full bg-zinc-900 border ${isTimeInvalid ? 'border-red-500' : 'border-white/5'} p-3 rounded-xl text-[11px] text-white font-bold outline-none focus:border-red-600 transition-colors [color-scheme:dark]`} 
              />
            </div>
          </div>
          ...
          {isTimeInvalid && (
            <div className="flex items-center gap-2 text-red-500 bg-red-500/10 p-3 rounded-xl animate-in fade-in slide-in-from-top-1">
              <AlertCircle size={14} />
              <p className="text-[10px] font-black uppercase">Lỗi: Thời gian kết thúc phải sau ngày bắt đầu!</p>
            </div>
          )}
          ...
          <div className="pt-4">
            <button 
              type="submit" 
              disabled={!canSubmit}
              className="w-full bg-white disabled:bg-zinc-800 text-black disabled:text-zinc-600 py-4 rounded-2xl font-black uppercase text-[11px] tracking-[0.2em] transition-all flex items-center justify-center gap-2 active:scale-95 shadow-lg shadow-white/5"
            >
              {isSubmitting ? (
                <Loader2 className="animate-spin" size={18} />
              ) : (
                <><Save size={18} /> {initialData ? "Cập nhật mã" : "Xác nhận tạo mã"}</>
              )}
            </button>
          </div>
        </form>
