    ...
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 backdrop-blur-md bg-black/60 animate-in fade-in duration-200">
      ...
      <div className="relative bg-[#0f0f0f] border border-zinc-900 w-full max-w-[380px] rounded-xl p-6 text-center space-y-5 shadow-2xl animate-in zoom-in-95 duration-200">
        ...
        <div className="space-y-1">
          <h2 className="text-base font-bold uppercase text-white tracking-tight">Xóa mục này?</h2>
          <p className="text-xs text-zinc-500 font-medium px-2 leading-relaxed">
            Dữ liệu về cụm <span className="text-zinc-200 font-semibold">"{title}"</span> sẽ bị xóa vĩnh viễn khỏi hệ thống hiển thị.
          </p>
        </div>
...
export default function CinemaDetailPage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await apiSuperAdminRequest(`/api/v1/cinema-items/${deleteTarget.id}`, { method: 'DELETE' });
      if (res.ok) {
      ...
      toast.error("Thao tác thất bại", { id: t }); 
    }
  };
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#020202] flex flex-col items-center justify-center gap-3">
      ...
      <span className="text-[10px] font-bold tracking-widest text-zinc-600 uppercase animate-pulse">Đang đồng bộ cấu trúc hệ thống...</span>
    </div>
  );
        ...
        <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-zinc-900 pb-6 gap-4">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <p className="text-zinc-600 font-bold text-[10px] uppercase tracking-widest">Infrastructure Hub</p>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <div className="space-y-4 relative z-10">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-2.5 bg-zinc-950/60 border border-zinc-900 rounded-lg">
                    <p className="text-[8px] font-black uppercase text-zinc-600 tracking-wider mb-0.5">Khu vực</p>
                    <p className="text-xs font-bold text-zinc-400 uppercase truncate">{item.city || "N/A"}</p>
                  </div>
                  <div className="p-2.5 bg-zinc-950/60 border border-zinc-900 rounded-lg">
                    <p className="text-[8px] font-black uppercase text-zinc-600 tracking-wider mb-0.5">Capacity</p>
                    <p className="text-xs font-bold text-zinc-400 uppercase">{item.hoursPerRoom || 0}H/D</p>
                  </div>
                </div>
          ...
          {cinemaItems.length === 0 && (
            <div className="col-span-full py-20 text-center border border-zinc-900 border-dashed bg-zinc-950/40 rounded-xl">
              <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-600">Trống - Chờ khởi tạo dữ liệu đơn vị cơ sở</p>
            </div>
          )}
        </div>
      </div>
