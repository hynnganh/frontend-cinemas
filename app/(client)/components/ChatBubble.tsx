...
import { MessageCircle, X, Send, Bot, UserSquare, Loader2, Film, ArrowRight } from 'lucide-react';
import Link from 'next/link'; // Import Link của Next.js để chuyển trang mượt
...
export default function ChatBubble() {
  ...
  const connectWebSocket = () => {
    ...
    client.onConnect = function () {
      ...
      fetch(`${BACKEND_URL}/api/v1/chat/history/${roomId}`)
        .then(res => res.json())
        .then(data => {
  ...
  // 🔥 BỘ PARSER MA THUẬT: Biến chuỗi văn bản thành UI 3D
  const renderMessageContent = (text: string, role: string) => {
    // Nếu là khách hàng gõ, chỉ in ra text bình thường
    if (role === "USER") {
      return text.split('\n').map((item, i) => <span key={i}>{item}<br/></span>);
    }

    // Nếu là AI (BOT), phân tích các mã bí mật $$MOVIE$$ và $$SEEMORE$$
    ...
    return parts.map((part, index) => {
      ...
      return <span key={index}>{part.split('\n').map((item, i) => <React.Fragment key={i}>{item}<br/></React.Fragment>)}</span>;
    });
  };
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[9999] flex flex-col items-end">
      ...
      <div 
        className={`mb-4 w-[360px] bg-zinc-950/95 backdrop-blur-xl border border-zinc-800/80 rounded-[2rem] shadow-[0_30px_60px_-15px_rgba(0,0,0,0.9)] flex flex-col overflow-hidden transition-all duration-500 origin-bottom-right transform ${
          isOpen ? "scale-100 opacity-100 translate-y-0" : "scale-50 opacity-0 pointer-events-none translate-y-10"
        }`}
        style={{ height: "520px" }}
      >
        <div className="bg-gradient-to-r from-red-600 to-rose-700 p-4 flex justify-between items-center relative overflow-hidden shadow-lg">
          ...
          <div className="flex items-center gap-3 relative z-10">
            ...
            <div>
              ...
              <p className="text-[10px] text-red-100 font-bold flex items-center gap-1.5 tracking-wider mt-0.5">
                ...
                {isConnecting ? "ĐANG ĐỒNG BỘ..." : "TRỰC TUYẾN"}
              </p>
            </div>
          </div>
          <button onClick={() => setIsOpen(false)} className="w-8 h-8 flex items-center justify-center rounded-full bg-black/20 text-white hover:bg-black/40 transition-all relative z-10">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
          <div className="text-center mb-6">
            <span className="text-[8px] bg-zinc-900/80 px-4 py-1.5 rounded-full text-zinc-500 font-black border border-zinc-800 uppercase tracking-[0.2em] shadow-inner">
              Mã hóa đầu cuối 256-bit
            </span>
          </div>
          
          <div className="flex flex-col items-start">
            <div className="max-w-[85%] bg-zinc-900/80 border border-zinc-800 text-zinc-300 text-[13px] px-4 py-3 rounded-2xl rounded-tl-sm shadow-md leading-relaxed">
              Kính chào quý khách! Tôi là siêu AI của <strong className="text-red-500">A&K Cinema</strong>. Tôi có thể tư vấn phim, lịch chiếu, giá vé hoặc tâm sự với bạn về mọi thứ trên đời. Bạn cần hỗ trợ gì ạ?
            </div>
          </div>
            ...
            return (
              <div key={idx} className={`flex flex-col ${isMe ? "items-end" : "items-start"} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
                ...
                <div className={`max-w-[88%] text-[13px] px-4 py-3 shadow-md leading-relaxed ${
                  ...
                  {/* Bắt đầu phân tích văn bản thành Giao diện phim */}
        ...
        <form onSubmit={sendMessage} className="p-3 bg-zinc-950/80 backdrop-blur-xl border-t border-zinc-800/80 flex items-center gap-2">
          <input 
            ...
            placeholder={chatMode === "BOT" ? "Hỏi lịch chiếu, phim mới, giải toán..." : "Nhập vấn đề gọi quản lý..."}
